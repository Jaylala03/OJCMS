...
namespace eCMS.Web.Areas.CaseManagement.Controllers
{
    public class CaseMemberContactController : CaseBaseController
    {
        ...
        public ActionResult IndexAjax([DataSourceRequest] DataSourceRequest dsRequest, string caseMemberID)  		        
        {
            ...
            FilterDescriptor newDesc = new FilterDescriptor("CaseMemberID", FilterOperator.IsEqualTo, caseMemberID);
                ...
                DataSourceResult result = casemembercontactRepository.AllIncluding(caseMemberID.ToInteger(true), casemembercontact => casemembercontact.CreatedByWorker, casemembercontact => casemembercontact.LastUpdatedByWorker, casemembercontact => casemembercontact.CaseMember, casemembercontact => casemembercontact.ContactMedia).Select(
                    casemembercontact => new { casemembercontact.ID, casemembercontact.CreateDate, casemembercontact.LastUpdateDate, 
                    ...
                    LastUpdatedByWorkerName = casemembercontact.LastUpdatedByWorker.FirstName + " " + casemembercontact.LastUpdatedByWorker.LastName,
                                               ...
                                               casemembercontact.IsPrimary,
                                               casemembercontact.CaseMemberID,
                                               casemembercontact.ContactMediaID, 
                    ContactMediaName = casemembercontact.ContactMedia.Name, casemembercontact.Contact, 
                    casemembercontact.Comments, 
                    casemembercontact.EmergencyContactNumber, 
                    casemembercontact .EmergencyContactName}).ToDataSourceResult(dsRequest);
        ...
        [WorkerAuthorize]
        [HttpPost]
        public ActionResult Save([DataSourceRequest] DataSourceRequest request, CaseMemberContact casemembercontact, int caseMemberID)
        {
            bool isNew = casemembercontact.ID == 0;
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    string mediaid = casemembercontact.ContactMediaID.ToString();
                    casemembercontact.CaseMemberID = caseMemberID;

                    casemembercontact.ContactMediaName = contactmediaRepository.AllActiveForDropDownList.Where(x => x.Value == mediaid).Select(x => x.Text).FirstOrDefault();
        ...
        [WorkerAuthorize]
        [HttpPost]
        public ActionResult Delete([DataSourceRequest] DataSourceRequest request, CaseMemberContact casemembercontact)
        {
            //find the casemembercontact in database
            //CaseMemberContact casemembercontact = casemembercontactRepository.Find(id);
            if (casemembercontact == null)
